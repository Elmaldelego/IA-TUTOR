...
import { and, count, desc, eq, getTableColumns, gte, like, lte, sql } from 'drizzle-orm'
...
export namespace Chats {
    ...
    export namespace db {
        export namespace query {
            ...
            export const chatsBetween = async (startDate: number, endDate: number) => {
                return await database.query.chats.findMany({
                    where: and(
                        gte(chats.create_date, new Date(startDate)),
                        lte(chats.create_date, new Date(endDate))
                    ),
                    with: {
                        messages: {
                            with: {
